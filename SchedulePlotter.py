...
def plot_stacked_bar(data, cutoff_value, m, map_size_to_round, show_values=False):
    ind = list(range(m))
...
def plot_schedule_for_subround(sub_round, map_size_to_round):
    plot_schedule(sub_round.schedule, sub_round.name, sub_round.m, sub_round.cutoff_value, map_size_to_round)


def plot_schedule(schedule: [[Fraction]], figure_name: str, m: int, cutoff_value: Fraction = None, map_size_to_round = None):
    ...
    plot_stacked_bar(layers, cutoff_value, m, map_size_to_round)
